...
class PostFeed extends Component {
    ...
    fetchLastestFeed(){
        if(this.state.nomore === false){
            ...
            setTimeout(() => {
                axios.get(`/api/post/status/${this.state.currentPage}/${this.state.lastId}`)
                    .then(res => {
        ...
        if(true){
            renderFeed = this.state.feeds.map((data,key) => (
                <React.Fragment key={key}>
                    <DivPosted>
                        <DivPostedProfile>
                            <LineName><ImgUser src={this.props.currentprofileurl} roundedCircle /> {this.props.currentname}</LineName>
                            <LineDate><Moment fromNow>{data.date}</Moment></LineDate>
                            {/* <Delete onClick={() => this.removePost(`${data._id}`)} icon="trash-alt" /> */}
